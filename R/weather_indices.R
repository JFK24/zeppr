...
# #'   Tmin=c(4, 6, 11),
# #'   Tmax=c(12, 14, 20))
# #' mutate_day_temp_indices(data, Tmin, Tmax)
# #' mutate_day_temp_indices(data, Tmin, Tmax, indices=c("frost", "heating"))
# #' mutate_day_temp_indices(data, Tmin, Tmax, german.output=TRUE)
# #' # With pipes from magrittr:
# #' library(magrittr)
# #' data %>% mutate_day_temp_indices(Tmin, Tmax)
# #' @importFrom magrittr %>%
# #' @importFrom rlang :=
# # #' @importFrom rlang .data
# #' @export
# # ==============================================================================
# mutate_day_temp_indices <- function(
...
#   for (index in indices) {
#     df <- df %>%
#       dplyr::mutate({{index}}:=day_temp_index({{t.min}}, {{t.max}}, index))
#   }
#   return(df)
# }
