...
normalize_trap_data <- function(
  ...
  df <- df %>%
    # dplyr::mutate(id.198445789832=dplyr::row_number()) %>%                             # better use with_order()
    dplyr::mutate(id.198445789832= dplyr::with_order(order_by = {{date}}, fun = dplyr::row_number, x = {{date}})) %>%
    # dplyr::mutate(diff.198445789832 = as.numeric({{date}} - dplyr::lag({{date}}))) %>% # better use with_order()
